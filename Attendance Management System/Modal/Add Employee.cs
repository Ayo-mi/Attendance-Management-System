...
namespace Attendance_Management_System.Modal
{
    public partial class Add_Employee : Form
    {
        ...
        {
            if (!String.IsNullOrEmpty(fName.Text) && !String.IsNullOrEmpty(lName.Text) && !String.IsNullOrEmpty(emId.Text) && enrollment.Enrollerr.TemplateStatus == DPFP.Processing.Enrollment.Status.Ready && posit_combo.SelectedIndex > -1)
            {
                DBConnection dB = new DBConnection();
                int posit_id = posit_combo.SelectedIndex;
                string fname = fName.Text.Trim();
                string lname = lName.Text.Trim();
                String id = emId.Text.Trim();
                byte[] sample = null;
