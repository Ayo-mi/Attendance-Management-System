...
namespace Attendance_Management_System.Modal.Edit_Modal
{
    public partial class Edit_Employee : Form
    {
        ...
        private void bunifuCheckBox1_CheckedChanged(object sender, Bunifu.UI.WinForms.BunifuCheckBox.CheckedChangedEventArgs e)
        {
            ...
            {
                samplelbl.Visible = false;
                enrollment.Stop();
            }
            else
            {
                samplelbl.Visible = true;
                enrollment.Start();
        ...
        {
            if (!String.IsNullOrEmpty(fName.Text) && !String.IsNullOrEmpty(lName.Text) && !String.IsNullOrEmpty(emId.Text) && posit_combo.SelectedIndex > -1)
            {
                DBConnection dB = new DBConnection();
                ...
                if (bunifuCheckBox1.Checked)
                {
                    stmt = "Update employees set first_name=@fn, last_name=@ln, employee_id=@idd, position_id=@po where employee_id=@id;";
                    attr.Add("@id", id);
                    attr.Add("@idd", emp);
                    attr.Add("@fn", fn);
                    attr.Add("@ln", ln);
                ...
                {
                    if(enrollment.Enrollerr.TemplateStatus == DPFP.Processing.Enrollment.Status.Ready)
                    {
                    ...
                    {
                        MessageBox.Show("Complete fingerprint re-enrollment before updating data", "Update Employee Data", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return;
                    }
